...
namespace Enemy.Boar
{
    public class BoarAttack : IAttackBehaviour
    {
        ...
        {
            this._owner = owner;
            var aliveCharacters = ControllerGamePlay.Instance.CurrentCharacter.Where(t => t.Alive).ToList();
            if (aliveCharacters.Count > 0)
            {
                _target = aliveCharacters[Random.Range(0, aliveCharacters.Count)];
            }
        }
        ...
        {
            ObseverAnimationEvent.Instance.Timer = 0.8f;
